...
The app now supports prerecorded Polly MP3 files, with Apple speech as a fallback. The production catalog contains 823 distinct lines across the six game categories, including all 60 storybook pages, game directions, feedback, and the parent preview. Ruth Generative is the default; 74 pronunciation-sensitive lines use Neural synthesis with explicit pronunciation markup. Pacing varies for stories, clear instructions, playful prompts, and gentle encouragement.

The current library contains **823 Ruth MP3 recordings**, totaling 18,429,764 bytes (18.4 MB). Playful prompts use natural pacing, clear directions use 95% speed, and stories/gentle activities retain slower delivery. The parent greeting and game-completion clips reuse the exact conversational Ruth samples selected by the user. Explicit letter-pronunciation markup remains intact.
...
The generator validates source text, caps estimated synthesis spending, saves a hash-verified cache, and publishes the app manifest only after the complete requested batch succeeds. Automated tests cover these safeguards and catalog coverage. The native audio test successfully opened all 823 bundled MP3s and verified their durations. UI route tests check recorded prompt coverage during tested interactions. Human listening remains necessary to judge warmth and pronunciation; catalog entries are not marked as human-reviewed automatically.
